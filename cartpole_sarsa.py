...
with tf.device('/GPU:0'):
    ...
    class SARSA:
        ...
        def selectAction(self, state, index):
            ...
            if randomNumber < self.epsilon:
                ...
                return np.random.choice(np.where(self.Qmatrix[self.returnIndexState(state)]==
                                                 np.max(self.Qmatrix[self.returnIndexState(state)]))[0])
    ...
    sarsa = SARSA(env,alpha,gamma,epsilon,num_epsiodes,numberOfBins,lowerBounds,upperBounds)
